// Regular expressions for checking and formatting the structure format string
const rechk = /^([<>])?(([1-9]\d*)?([xcbB?hHiIfdsp]))*$/
const refmt = /([1-9]\d*)?([xcbB?hHiIfdsp])/g

// Function to convert a TypedArray to a string
const str = (v, o, c) => String.fromCharCode(
    ...new Uint8Array(v.buffer, v.byteOffset + o, c))

// Function to create a TypedArray from a string
const rts = (v, o, c, s) => new Uint8Array(v.buffer, v.byteOffset + o, c)
    .set(s.split('').map(char => char.charCodeAt(0)))

// Function to read a string from a TypedArray
const pst = (v, o, c) => str(v, o + 1, Math.min(v.getUint8(o), c - 1))

// Function to write a string to a TypedArray
const tsp = (v, o, c, s) => {
...
// Exported function for parsing and formatting structured data
export default function struct(format) {
    ...
    const isLittleEndian = '<' === m[1];
    const t = lut(isLittleEndian);
